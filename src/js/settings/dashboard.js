($ => {
    ...
    $.DashboardHelper = function (s) {
        ...
        const initTipsAndTricks = async () => {
            const ctrlKeyLabel = $.ua().os === "macOS" ? "cmd" : "ctrl";
            ...
            await displayTipTrick(randomListOfTipsTricks[idx], tipsTricks[randomListOfTipsTricks[idx]]);

            s.elm.dashboard.tipsTricks.children("button").on("click", async (e) => { // show the next tip/trick from the shuffled list
                e.preventDefault();
                idx = (idx + 1) % randomListOfTipsTricks.length;
                const uid = randomListOfTipsTricks[idx];
                await displayTipTrick(uid, tipsTricks[uid]);
            });
        };
         ...
         * @param obj
         */
        const displayTipTrick = async (uid, obj) => {
            ...
            if (obj.img) {
                ...
                const videoHref = await s.helper.model.call("parsedUrl", {
                    href: obj.video
                });
